...
const ATTENDANCE_STYLES: Record<AttendanceStatus, string> = {
  "Hadir":       "bg-green-700  border-green-700  hover:border-green-500",
  "Tidak Hadir": "bg-red-700    border-red-700    hover:border-red-500",
  "Masih Ragu":  "bg-yellow-700 border-yellow-700 hover:border-yellow-500",
};

const INACTIVE_STYLE = "bg-zinc-900 border-zinc-700";

export default function RSVP() {
  const API = process.env.NEXT_PUBLIC_RSVP_API!;
  ...
  const submitRSVP = async () => {
    if (!name.trim())    { toast.error("Nama belum terisi");           return; }
    if (!message.trim()) { toast.error("Ucapan & Doa belum terisi");   return; }
  ...
  const getStatusIcon = (status: string) => {
    if (status === "Hadir")       return <CheckCircle2 size={18} className="text-green-500"  />;
    if (status === "Tidak Hadir") return <XCircle      size={18} className="text-red-500"    />;
    return                               <HelpCircle   size={18} className="text-yellow-500" />;
  };

  const totalPages     = Math.ceil(wishes.length / ITEMS_PER_PAGE) || 1;
  ...
  return (
    <section className="py-24 px-6">
      <h2 className="text-4xl font-bold text-center mb-12">RSVP & Ucapan</h2>
      ...
      <div className="grid lg:grid-cols-2 gap-10 max-w-7xl mx-auto">
        ...
        <div className="bg-[#141414] border border-red-900/30 rounded-2xl p-8">
          ...
          <div className="mb-5">
            <p className="text-gray-400 mb-3">Status Kehadiran</p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {(Object.keys(ATTENDANCE_STYLES) as AttendanceStatus[]).map((status) => (
                <button
                  key={status}
                  type="button"
                  ...
                  className={`py-3 rounded-xl border font-semibold transition ${
                    attendance === status
                      ...
                      : `${INACTIVE_STYLE} ${ATTENDANCE_STYLES[status].split(" ").at(-1)}`
                  }`}
                >
                  {status}
                </button>
        ...
        {/* ── UCAPAN ── */}
        <div className="bg-[#141414] border border-red-900/30 rounded-2xl p-8">
          <h3 className="text-2xl font-bold mb-6">Ucapan & Doa</h3>
            ...
            {paginatedWishes.map((wish, index) => (
              <div key={index} className="border-b border-zinc-800 pb-5">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {getStatusIcon(wish.attendance)}
                </div>
                <p className="text-gray-300 whitespace-pre-wrap">{wish.message}</p>
              </div>
            ))}
          </div>
